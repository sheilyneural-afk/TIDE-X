...
use crate::linalg::{cosine, dot, norm, Matrix};
use crate::validation::independence_group_folds;
    ...
    for r in 0..d.rows {
        ...
        if !n.is_finite() || n <= 1e-15 {
            return Err(BrainError::Invalid(format!(
                "persistent_zero_or_invalid_row:{r}"
            )));
        }
...
fn cluster_rows(rows: &[Vec<f64>], groups: &[String]) -> BrainResult<ClusterSolution> {
    if rows.len() < 2 {
        return Err(BrainError::Invalid(
            "persistent_cluster_minimum_rows".into(),
        ));
    }
    let dim = rows[0].len();
    if dim == 0 || rows.iter().any(|row| row.len() != dim) {
        return Err(BrainError::Invalid(
            "persistent_cluster_dimension_mismatch".into(),
        ));
    }
    if groups.len() != rows.len() || groups.iter().any(|group| group.trim().is_empty()) {
        return Err(BrainError::Invalid(
            "persistent_cluster_group_count_mismatch".into(),
        ));
    }
    ...
    if similarities.len() < 2 {
        return Err(BrainError::Invalid(
            "persistent_cluster_insufficient_pairs".into(),
        ));
    }
...
fn centroid_for_members(
    ...
    members: &[usize],
    reliability: &[f64],
) -> BrainResult<(Vec<f64>, Vec<f64>, f64)> {
    if members.is_empty() {
    ...
    let mut raw_weights = Vec::with_capacity(members.len());
    ...
    for &index in members {
        ...
        let weight = reliability[index].clamp(1e-4, 1.0);
        total_weight += weight;
        raw_weights.push((index, sign, weight));
        for p in 0..dim {
    ...
    if total_weight <= 1e-15 {
        return Err(BrainError::Numerical(
            "persistent_cluster_weight_zero".into(),
        ));
    }
    ...
    if !pre_norm.is_finite() || pre_norm <= 1e-15 {
        return Err(BrainError::Numerical(
            "persistent_cluster_centroid_degenerate".into(),
        ));
    }
    let direction = pre.iter().map(|value| value / pre_norm).collect::<Vec<_>>();
    let mut member_coefficients = vec![0.0; rows.len()];
    for (index, sign, weight) in raw_weights {
        // These coefficients apply to already normalized rows. Conversion to
        // original delta rows is performed by the caller using each row norm.
        member_coefficients[index] = sign * weight / total_weight / pre_norm;
    }
    Ok((direction, member_coefficients, pre_norm))
}
...
fn functional_dimension(observations: &[DeltaObservation]) -> BrainResult<usize> {
    ...
    if dim == 0
        || observations
            .iter()
            .any(|observation| observation.functional_response.len() != dim)
    {
        return Err(BrainError::Invalid(
            "persistent_functional_dimension_invalid".into(),
        ));
    }
    Ok(dim)
}
...
fn weighted_functional_signature(
    ...
    if rows.len() != observations.len() || members.is_empty() {
        return Err(BrainError::Invalid(
            "persistent_functional_signature_shape".into(),
        ));
    }
    let reference = &rows[members[0]];
    ...
    let mut total = 0.0;
    for &index in members {
        let w = observations[index].reliability.clamp(1e-4, 1.0);
        let sign = if cosine(reference, &rows[index])? >= 0.0 {
            1.0
        } else {
            -1.0
    ...
    if total <= 1e-15 {
        return Err(BrainError::Numerical(
            "persistent_functional_weight_zero".into(),
        ));
    }
...
fn assignment_min_margin(
    ...
    if reference.len() != assignment.len() || candidate.is_empty() {
        return Err(BrainError::Invalid(
            "persistent_identity_margin_shape".into(),
        ));
    }
    ...
    if !minimum.is_finite() {
        return Err(BrainError::Numerical(
            "persistent_identity_margin_nonfinite".into(),
        ));
    }
    Ok(minimum)
}
...
fn cross_aperture_functional_cv(
    ...
    for fold in &folds {
        ...
        if train_indices.len() < 4 {
            return Err(BrainError::Invalid(
                "persistent_cv_fold_cardinality_invalid".into(),
            ));
        }
        ...
        for component in &solution.components {
            let (centroid, _, _) =
                centroid_for_members(&train_rows, component, &train_reliability)?;
            let original_members = component
                .iter()
                .map(|&local| train_indices[local])
                .collect::<Vec<_>>();
            centroids.push(centroid);
            signatures.push(weighted_functional_signature(
                rows,
                observations,
                &original_members,
            )?);
    ...
    if predicted.iter().any(|value| !*value) {
        return Err(BrainError::Integrity(
            "persistent_cv_incomplete_prediction_coverage".into(),
        ));
    }
...
pub fn reconstruct_persistent_skill_fields(
    ...
    if d.rows != observations.len() || d.rows < minimum_observations || d.cols == 0 {
        return Err(BrainError::Invalid(
            "persistent_tomography_input_shape".into(),
        ));
    }
    ...
    let all_groups = observations
        ...
        .map(|observation| observation.independence_group.clone())
        .collect::<BTreeSet<_>>();
    let total_raw_energy = (0..d.rows)
        .try_fold(0.0, |total, row| {
            Ok::<f64, BrainError>(
                total + reliability[row].clamp(1e-4, 1.0) * dot(d.row(row), d.row(row))?,
            )
        })?
        .max(1e-18);
    ...
    for members in &solution.components {
        let (direction, normalized_mixture, _) =
            centroid_for_members(&rows, members, &reliability)?;
        ...
        let total_materialization_weight = members
            .iter()
            .map(|&member| reliability[member].clamp(1e-4, 1.0))
            .sum::<f64>()
            .max(1e-15);
        ...
        for &member in members {
            ...
            original_mixture[member] =
                sign * reliability[member].clamp(1e-4, 1.0) / total_materialization_weight;
        }
        let _ = normalized_mixture;
        ...
        energies.push(cluster_energy.max(1e-18));
        let support_weight = members
            .iter()
            .map(|&row| reliability[row].clamp(1e-4, 1.0))
            .sum::<f64>()
            .max(1e-18);
        let coherence = members.iter().try_fold(0.0, |total, &row| {
            Ok::<f64, BrainError>(
                total + reliability[row].clamp(1e-4, 1.0) * cosine(&rows[row], &direction)?.abs(),
            )
        })? / support_weight;
        let angular_variance = members.iter().try_fold(0.0, |total, &row| {
            let residual = 1.0 - cosine(&rows[row], &direction)?.abs().clamp(0.0, 1.0);
            Ok::<f64, BrainError>(total + reliability[row].clamp(1e-4, 1.0) * residual * residual)
        })? / support_weight;
    ...
    for row in 0..d.rows {
        ...
        for (field_index, direction) in directions.iter().enumerate() {
            let coefficient = dot(d.row(row), direction)?;
    ...
    let normalized_reconstruction_rms = (residual_energy / input_energy.max(1e-18)).sqrt();
    let smallest = energies
        .iter()
        .copied()
        .fold(f64::INFINITY, f64::min)
        .max(1e-18);
    let largest = energies
        .iter()
        .copied()
...
mod tests {
    ...
    fn structural_threshold_selection_avoids_largest_gap_fragmentation() {
        ...
        assert_eq!(
            solution.components.iter().map(Vec::len).collect::<Vec<_>>(),
            vec![3, 3]
        );
    ...
    fn persistent_inverse_discovers_recurrent_skills_without_task_labels() {
        ...
        assert!(
            result.functional_cv_r2 > 0.95,
            "{}",
            result.functional_cv_r2
        );
        assert!(
            result.cluster_stability > 0.99,
            "{}",
